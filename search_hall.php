<html>
    <head>
    <title>Hall Search Results</title></head>
    <body>
    <?php
    ...
    $area = $_POST['area'];
    $minCost = $_POST['min_cost'];
    $maxCost = $_POST['max_cost'];
    $headCount = $_POST['headCount'];
    $booking_date = $_POST['date'];
    $formatted_booking_date = date('d-m-Y', strtotime($booking_date));
    $booking_slot = $_POST['slot'];
    //echo $booking_date;
    //echo $booking_slot;
    //echo $area;
    //echo $formatted_booking_date;
    //echo $headCount;

    $query = "SELECT * FROM hall WHERE hall_capacity >= :headCount AND
        hall_id NOT IN (
        SELECT hall_id FROM books
        WHERE booking_date = TO_DATE(:formatted_booking_date, 'DD-MM-YYYY')
        AND booked_slot = :booking_slot
        )";

    if (!empty($area)) {
        $query .= " AND hall_area = :area";
    }
    if (!empty($minCost)) {
        $query .= " AND hall_cost >= :minCost";
    }
    if (!empty($maxCost)) {
        $query .= " AND hall_cost <= :maxCost";
    }

    $query .= " ORDER BY hall_cost";

    $statement = oci_parse($connection, $query);

    oci_bind_by_name($statement, ':headCount', $headCount);
    //oci_bind_by_name($statement, ':hall_area', $hall_area);
    oci_bind_by_name($statement, ':formatted_booking_date', $formatted_booking_date);
    oci_bind_by_name($statement, ':booking_slot', $booking_slot);
    if (!empty($area)) {
        oci_bind_by_name($statement, ':area', $area);
    }
    if (!empty($minCost)) {
        oci_bind_by_name($statement, ':minCost', $minCost);
    }
    if (!empty($maxCost)) {
        oci_bind_by_name($statement, ':maxCost', $maxCost);
    }
    ...
    $tableHtml = '<table>
        ...
        while ($row = oci_fetch_assoc($statement)) {
            $tableHtml .= '<tr>';
            $tableHtml .= '<td style="text-align: left;">' . $row['HALL_NAME'] . '</td>';
            $tableHtml .= '<td style="text-align: left;">' . $row['HALL_CAPACITY'] . '</td>';
            $tableHtml .= '<td style="text-align: left;">' . $row['HALL_COST'] . '</td>';
            $tableHtml .= '<td style="text-align: left;">' . $row['HALL_CITY'] . '</td>';
            $tableHtml .= '<td style="text-align: left;">' . $row['HALL_AREA'] . '</td>';
            $tableHtml .= '<td style="text-align: left;">' . $row['HALL_STREET'] . '</td>';
            $tableHtml .= '<td style="text-align: left;">' . $row['HALL_HOUSE_NO'] . '</td>';
            $tableHtml .= '<td>
                                <form action="book_this_hall.php" method="post">
    ...
    $query = "update temp set head_count=:headCount where id=1";
    $statement = oci_parse($connection, $query);
    oci_bind_by_name($statement, ':headCount', $headCount);
    oci_execute($statement);

    $query = "update temp set booking_date=TO_DATE(:formatted_booking_date, 'DD-MM-YYYY')";
    $statement=oci_parse($connection, $query);
    oci_bind_by_name($statement, ':formatted_booking_date', $formatted_booking_date);;
    oci_execute($statement);

    $query = "update temp set booking_slot=:booking_slot where id=1";
    $statement = oci_parse($connection, $query);
    oci_bind_by_name($statement, ':booking_slot', $booking_slot);
    oci_execute($statement);
...
<div class="fables-header fables-after-overlay" style="background-image: url(assets/custom/images/index1bg.jpg);">
    <div class="container"> 
         <h2 class="fables-page-title fables-second-border-color">Hall Search Results</h2>
    </div>
</div>  
<!-- /End Header -->
    ...
    <div class="container"> 
        <nav aria-label="breadcrumb">
          <ol class="fables-breadcrumb breadcrumb px-0 py-3">
            ...
            <li class="breadcrumb-item active" aria-current="page">Hall Search Results</li>
          </ol>
        </nav> 
    </div>
</div>
...
<!-- Start page content -->
        <h1>Hall Search Results</h1><br><br>
    
        <?php echo $tableHtml; ?><br><br>
<!-- /End page content -->
