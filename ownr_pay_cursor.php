...
<!-- Start Header -->
<div class="fables-header fables-after-overlay" style="background-image: url(assets/custom/images/PaymentBg.jpg);">
    <div class="container"> 
         <h2 class="fables-page-title fables-second-border-color">Payment List</h2>
    </div>
</div>  
<!-- /End Header -->
...
//Define the cursor query
$query = "
DECLARE
    ownr_pay_cursor SYS_REFCURSOR;
BEGIN
    OPEN ownr_pay_cursor FOR
        ...
        WHERE b.status = 'Booked' AND o.owner_id = :owner_id
...
// Execute the cursor variable
oci_execute($cursor);

// Fetch the cursor data
while (($row = oci_fetch_array($cursor, OCI_ASSOC + OCI_RETURN_NULLS)) !== false) {
    echo 'Payment Date: ' . $row['Payment Date'] . '<br>';
    echo 'Payment No: ' . $row['PAYMENT NO'] . '<br>';
    echo 'Customer Name: ' . $row['Customer Name'] . '<br>';
    echo 'Hall Name: ' . $row['Hall Name'] . '<br>';
    echo 'Booking Slot: ' . $row['Booking Slot'] . '<br>';
    echo 'Total Payment: ' . $row['Total Payment'] . '<br>';
    echo '------------------------------<br>';
}
