<html>
    ...
    <?php
        ...
        $query = "update temp set hall_id=:hallID, menu_id=:menuID, dec_id=:decID, catering_id=:cateringID where id=1";
        $statement = oci_parse($connection, $query);
        oci_bind_by_name($statement, ':hallID', $hallID);
        oci_bind_by_name($statement, ':menuID', $menuID);
        oci_bind_by_name($statement, ':decID', $decID);
        oci_bind_by_name($statement, ':cateringID', $cateringID);
        oci_execute($statement);

        $query = "SELECT hall_cost FROM hall WHERE hall_id = (select hall_id from temp where id=1)";
        $statement = oci_parse($connection, $query);
        oci_execute($statement);
        $hallPrice = 0;
        if ($row = oci_fetch_assoc($statement)) {
            $hallPrice = $row['HALL_COST'];
        }

        $query = "SELECT dec_price FROM decorations WHERE dec_id = (select dec_id from temp where id=1)";
        $statement = oci_parse($connection, $query);
        oci_execute($statement);
        $decPrice = 0;
        if ($row = oci_fetch_assoc($statement)) {
            $decPrice = $row['DEC_PRICE'];
        }

        $query = "SELECT m_price FROM menu WHERE menu_id = (select menu_id from temp where id=1)";
        $statement = oci_parse($connection, $query);
        oci_execute($statement);
        $menuPrice = 0;
        if ($row = oci_fetch_assoc($statement)) {
            $menuPrice = $row['M_PRICE'];
        }

        $query = "SELECT catering_price FROM catering_service WHERE catering_id = (select catering_id from temp where id=1)";
        $statement = oci_parse($connection, $query);
        oci_execute($statement);
        $cateringPrice = 0;
        if ($row = oci_fetch_assoc($statement)) {
            $cateringPrice = $row['CATERING_PRICE'];
        }
        ...
        $totalPrice=$hallPrice+$menuPrice*$headCount+$cateringPrice*$headCount+$decPrice;
        ...
        echo '
            ...
            <head>
                ...
                function go_back() {
                    window.location.href = "search_hall.html";
                }
                </script>
                <style>
                    .container1 {
...
<div class="fables-header fables-after-overlay" style="background-image: url(assets/custom/images/index1bg.jpg);">
    <div class="container"> 
         <h2 class="fables-page-title fables-second-border-color">Payment Details</h2>
    </div>
</div>  
<!-- /End Header -->
    ...
    <div class="container"> 
        <nav aria-label="breadcrumb">
          <ol class="fables-breadcrumb breadcrumb px-0 py-3">
            ...
            <li class="breadcrumb-item"><a href="search_hall.html" class="fables-second-text-color">Hall Search</a></li>
            <li class="breadcrumb-item"><a href="search_hall.php" class="fables-second-text-color">Hall Search Results</a></li>
            <li class="breadcrumb-item"><a href="book_this_hall.php" class="fables-second-text-color">Hall Details</a></li>
            <li class="breadcrumb-item"><a href="search_menu.php" class="fables-second-text-color">Available Menu</a></li>
            <li class="breadcrumb-item"><a href="search_decoration.php" class="fables-second-text-color">Available Decorations</a></li>
...
<!-- Start page content -->
                <div class="container1">
                    ...
                    <p><strong>Decoration Price:</strong> ' . $decPrice . '</p>
                    <p><strong>Menu Price:</strong> ' . $menuPrice . '</p>
                    <p><strong>Catering Price:</strong> ' . $cateringPrice . '</p>
                    <p><strong>Head Count:</strong> ' . $headCount . '</p>
                    <p><strong>Total Price:</strong> ' . $totalPrice . '</p>
                </div>
                <form action="insert_payment.php" method="POST" align="center">
                    <input type="hidden" name="amount" value="' . $totalPrice . '">
                    <input class="BH" type="submit" value="Confirm Payment and Proceed">
                </form>
                <form action="search_hall.php" align="center">
                    <input class="GB" type="submit" value="Go Back">
                </form>
<!-- /End page content -->
