<html>
    <head>
    <title>Hall Details</title></head>
    <body>
    <?php
    if (isset($_POST['hall_id'])) {
        // Retrieve the hall_id from the form submission
        $hallId = $_POST['hall_id'];

        $connection = oci_connect('HELLO', 'hello', 'localhost/XEPDB1');

        $query = 'SELECT * FROM hall WHERE hall_id = :hallId';
...
<div class="fables-header fables-after-overlay" style="background-image: url(assets/custom/images/index1bg.jpg);">
    <div class="container"> 
         <h2 class="fables-page-title fables-second-border-color">Hall Details</h2>
    </div>
</div>  
<!-- /End Header -->
...
<!-- Start page content -->
                <div class="container1">
                    <h2>Hall Details</h2>
                    <img src="' . $hall_pic . '" alt="Hall Image">
                    <p><strong>Name:</strong> ' . $row['HALL_NAME'] . '</p>
                    <p><strong>Cost:</strong> ' . $row['HALL_COST'] . '</p>
                    <p><strong>Capacity:</strong> ' . $row['HALL_CAPACITY'] . '</p>
                    <p><strong>Area:</strong> ' . $row['HALL_AREA'] . '</p>
                    <p><strong>Street:</strong> ' . $row['HALL_STREET'] . '</p>
                    <p><strong>House:</strong> ' . $row['HALL_HOUSE_NO'] . '</p>
                </div>
                <form action="search_menu.php" method="POST" align="center">
...
<script src="assets/custom/js/custom.js"></script>
            ...
            echo 'No hall found with the specified hall ID.';
        }
