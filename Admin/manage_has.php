<html>
    ...
    <body>
    <?php
// Check if the form is submitted and the decoration IDs are present
if (isset($_POST['decoration_ids']) && isset($_POST['hall_id'])) {
    // Retrieve the selected decoration IDs and the hall ID
    $decorationIds = $_POST['decoration_ids'];
    ...
    // Prepare the query to check existing records in the 'has' table
    $existingQuery = "SELECT * FROM has WHERE hall_id = :hallId";

    // Prepare the query to insert new records into the 'has' table
    $insertQuery = "INSERT INTO has (hall_id, dec_id) VALUES (:hallId, :decorationId)";

    // Prepare the query to delete existing records from the 'has' table
    $deleteQuery = "DELETE FROM has WHERE hall_id = :hallId AND dec_id = :decorationId";
...
<!-- Start page content -->
    ...
    foreach ($existingResult as $existingRecord) {
        $existingdecorationId = $existingRecord['DEC_ID'];

        // Check if the existing decoration ID is in the selected decoration IDs array
        if (in_array($existingdecorationId, $decorationIds)) {
            // decoration ID exists in the selected decoration IDs array, do nothing
            continue;
        } else {
            // decoration ID does not exist in the selected decoration IDs array, delete the record
            $deleteStatement = oci_parse($connection, $deleteQuery);
            oci_bind_by_name($deleteStatement, ':hallId', $hallId);
            oci_bind_by_name($deleteStatement, ':decorationId', $existingdecorationId);
            oci_execute($deleteStatement);

            echo "Deleted: Hall ID: " . $hallId . ", decoration ID: " . $existingdecorationId . "<br>";

            oci_free_statement($deleteStatement);
        }
    }

    // Loop through the selected decoration IDs
    foreach ($decorationIds as $decorationId) {
        // Check if the record exists in the 'has' table
        $existingRecord = array_filter($existingResult, function ($record) use ($decorationId) {
            return $record['DEC_ID'] === $decorationId;
        });
        ...
        if (count($existingRecord) > 0) {
            ...
            // Record doesn't exist, insert it into the 'has' table
            $insertStatement = oci_parse($connection, $insertQuery);
            oci_bind_by_name($insertStatement, ':hallId', $hallId);
            oci_bind_by_name($insertStatement, ':decorationId', $decorationId);
            oci_execute($insertStatement);

            echo "Inserted: Hall ID: " . $hallId . ", decoration ID: " . $decorationId . "<br>";
