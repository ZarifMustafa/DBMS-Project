<html>
    ...
    <body>
    <?php
// Check if the form is submitted and the owner IDs are present
if (isset($_POST['owner_ids']) && isset($_POST['hall_id'])) {
    // Retrieve the selected owner IDs and the hall ID
    ...
    // Prepare the query to check existing records in the 'owns' table
    $existingQuery = "SELECT * FROM owns WHERE hall_id = :hallId";

    // Prepare the query to insert new records into the 'owns' table
    $insertQuery = "INSERT INTO owns (hall_id, owner_id) VALUES (:hallId, :ownerId)";

    // Prepare the query to delete existing records from the 'owns' table
    $deleteQuery = "DELETE FROM owns WHERE hall_id = :hallId AND owner_id = :ownerId";
...
<!-- Start page content -->
    ...
    <p class="message-text1">
        ...
        foreach ($existingResult as $existingRecord) {
            $existingOwnerId = $existingRecord['OWNER_ID'];

            // Check if the existing owner ID is in the selected owner IDs array
            if (in_array($existingOwnerId, $ownerIds)) {
                // Owner ID exists in the selected owner IDs array, do nothing
                continue;
            } else {
                // Owner ID does not exist in the selected owner IDs array, delete the record
                $deleteStatement = oci_parse($connection, $deleteQuery);
                oci_bind_by_name($deleteStatement, ':hallId', $hallId);
                oci_bind_by_name($deleteStatement, ':ownerId', $existingOwnerId);
                oci_execute($deleteStatement);

                echo "Deleted: Hall ID: " . $hallId . ", Owner ID: " . $existingOwnerId . "<br>";

                oci_free_statement($deleteStatement);
            }
        }

        // Loop through the selected owner IDs
        foreach ($ownerIds as $ownerId) {
            // Check if the record exists in the 'owns' table
            $existingRecord = array_filter($existingResult, function ($record) use ($ownerId) {
            ...
            if (count($existingRecord) > 0) {
                ...
                // Record doesn't exist, insert it into the 'owns' table
                $insertStatement = oci_parse($connection, $insertQuery);
                oci_bind_by_name($insertStatement, ':hallId', $hallId);
                oci_bind_by_name($insertStatement, ':ownerId', $ownerId);
                oci_execute($insertStatement);

                echo "Inserted: Hall ID: " . $hallId . ", Owner ID: " . $ownerId . "<br>";
