<html>
    ...
    <body>
    <?php
// Check if the form is submitted and the catering IDs are present
if (isset($_POST['catering_ids']) && isset($_POST['hall_id'])) {
    // Retrieve the selected catering IDs and the hall ID
    $cateringIds = $_POST['catering_ids'];
    ...
    // Prepare the query to check existing records in the 'offers' table
    $existingQuery = "SELECT * FROM offers WHERE hall_id = :hallId";

    // Prepare the query to insert new records into the 'offers' table
    $insertQuery = "INSERT INTO offers (hall_id, catering_id) VALUES (:hallId, :cateringId)";

    // Prepare the query to delete existing records from the 'offers' table
    $deleteQuery = "DELETE FROM offers WHERE hall_id = :hallId AND catering_id = :cateringId";
...
<!-- Start page content -->
    ...
    foreach ($existingResult as $existingRecord) {
        $existingcateringId = $existingRecord['CATERING_ID'];

        // Check if the existing catering ID is in the selected catering IDs array
        if (in_array($existingcateringId, $cateringIds)) {
            // catering ID exists in the selected catering IDs array, do nothing
            continue;
        } else {
            // catering ID does not exist in the selected catering IDs array, delete the record
            $deleteStatement = oci_parse($connection, $deleteQuery);
            oci_bind_by_name($deleteStatement, ':hallId', $hallId);
            oci_bind_by_name($deleteStatement, ':cateringId', $existingcateringId);
            oci_execute($deleteStatement);

            echo "Deleted: Hall ID: " . $hallId . ", catering ID: " . $existingcateringId . "<br>";

            oci_free_statement($deleteStatement);
        }
    }

    // Loop through the selected catering IDs
    foreach ($cateringIds as $cateringId) {
        // Check if the record exists in the 'offers' table
        $existingRecord = array_filter($existingResult, function ($record) use ($cateringId) {
            return $record['CATERING_ID'] === $cateringId;
        });
        ...
        if (count($existingRecord) > 0) {
            ...
            // Record doesn't exist, insert it into the 'offers' table
            $insertStatement = oci_parse($connection, $insertQuery);
            oci_bind_by_name($insertStatement, ':hallId', $hallId);
            oci_bind_by_name($insertStatement, ':cateringId', $cateringId);
            oci_execute($insertStatement);

            echo "Inserted: Hall ID: " . $hallId . ", catering ID: " . $cateringId . "<br>";
