<html>
    ...
    // Retrieve all decorations from the database
    $query = 'SELECT * FROM decorations';
    $statement = oci_parse($connection, $query);
    oci_execute($statement);

    // Create an array to store the decorations
    $decorations = array();

    while ($row = oci_fetch_assoc($statement)) {
        $decorationId = $row['DEC_ID'];
        $decorationTheme = $row['THEME'];

        // Add the decoration to the array
        $decorations[$decorationId] = $decorationTheme;
    }
...
<head>
    ...
    <script>
        function validateForm() {
            var checkboxes = document.getElementsByName('decoration_ids[]');
            var isChecked = false;
            ...
            if (!isChecked) {
                alert("Please select at least one decoration.");
...
<!-- Start page content -->
    <div class="container text-center">
        <h2>Select Owners</h2>
        <form class="my-4" method="post" action="manage_has.php" onsubmit="return validateForm()">
            <input type="hidden" name="hall_id" value="<?php echo $hallId; ?>">
            <?php
            // Iterate over the decorations array and generate the checkboxes
            foreach ($decorations as $decorationId => $decorationTheme) {
                echo '<label class="checkbox-label">';
                echo '<input class="checkbox-input" type="checkbox" name="decoration_ids[]" value="' . $decorationId . '">' . $decorationId . ' - ' . $decorationTheme;
                echo '</label><br>';
