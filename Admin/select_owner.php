<html>
    ...
    // Create an array to store the owners
    $owners = array();
...
<!-- Start page content -->
    <div class="container text-center">
        <h2>Select Owners</h2>
        <form class="my-4" method="post" action="manage_owns.php" onsubmit="return validateForm()">
            <input type="hidden" name="hall_id" value="<?php echo $hallId; ?>">
            <?php
            // Iterate over the owners array and generate the checkboxes
            foreach ($owners as $ownerId => $ownerName) {
                echo '<label class="checkbox-label">';
                echo '<input class="checkbox-input" type="checkbox" name="owner_ids[]" value="' . $ownerId . '">' . $ownerName;
                echo '</label><br>';
            }
            ?>
