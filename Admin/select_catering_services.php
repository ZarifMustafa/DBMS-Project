<html>
    ...
    <body>
    <?php
    // Check if the hall_id is present in the URL parameters
    if (isset($_GET['hall_id'])) {
        // Retrieve the hall_id from the URL
        $hallId = $_GET['hall_id'];
        //echo $hallId;
    }
    ...
    // Retrieve all catering services from the database
    $query = 'SELECT * FROM catering_service';
    $statement = oci_parse($connection, $query);
    oci_execute($statement);

    // Create an array to store the catering servicess
    $caterings = array();
    ...
    while ($row = oci_fetch_assoc($statement)) {
        ...
        // Add the catering service to the array
        $caterings[$cateringId] = $cateringName;
    }
...
<head>
    ...
    <script>
        function validateForm() {
            var checkboxes = document.getElementsByName('catering_ids[]');
            var isChecked = false;
            ...
            if (!isChecked) {
                alert("Please select at least one catering service.");
...
<div class="fables-header fables-after-overlay" style="background-image: url(assets/custom/images/OwnerRegBg.jpg);">
    <div class="container"> 
         <h2 class="fables-page-title fables-second-border-color">Select Owner</h2>
    </div>
</div>  
<!-- /End Header -->
...
<!-- Start page content -->
    <div class="container text-center">
        <h2>Select Owners</h2>
        <form class="my-4" method="post" action="manage_offers.php" onsubmit="return validateForm()">
            <input type="hidden" name="hall_id" value="<?php echo $hallId; ?>">
            <?php
            // Iterate over the caterings array and generate the checkboxes
            foreach ($caterings as $cateringId => $cateringName) {
                echo '<label class="checkbox-label">';
                echo '<input class="checkbox-input" type="checkbox" name="catering_ids[]" value="' . $cateringId . '">' . $cateringName;
                echo '</label><br>';
            }
            ?>
