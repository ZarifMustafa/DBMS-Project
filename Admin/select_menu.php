...
<head>
    ...
    <script>
        function validateForm() {
            var checkboxes = document.getElementsByName('menu_ids[]');
            var isChecked = false;
...
<!-- Start page content -->
    <div class="container text-center">
        <h2>Select Owners</h2>
        <form class="my-4" method="post" action="manage_provides.php" onsubmit="return validateForm()">
            <input type="hidden" name="hall_id" value="<?php echo $hallId; ?>">
            <?php
            // Iterate over the menus array and generate the checkboxes
            foreach ($menus as $menuId => $menuName) {
                echo '<label class="checkbox-label">';
                echo '<input class="checkbox-input" type="checkbox" name="menu_ids[]" value="' . $menuId . '">' . $menuName;
                echo '</label><br>';
            }
            ?>
