<html>
    ...
    <body>
    <?php
    // Check if the menu_id is present in the URL parameters
    if (isset($_GET['menu_id'])) {
        // Retrieve the menu_id from the URL
        $menuId = $_GET['menu_id'];
        //echo $menuId;
    }
    ...
    // Create an array to store the foods
    $foods = array();
...
<head>
    ...
    <script>
        function validateForm() {
            var checkboxes = document.getElementsByName('food_ids[]');
            var isChecked = false;
...
<!-- Start page content -->
    <div class="container text-center">
        <h2>Select Food</h2>
        <form class="my-4" method="post" action="manage_contains.php" onsubmit="return validateForm()">
            <input type="hidden" name="menu_id" value="<?php echo $menuId; ?>">
            <?php
            // Iterate over the foods array and generate the checkboxes
            foreach ($foods as $foodId => $foodName) {
                echo '<label class="checkbox-label">';
                echo '<input class="checkbox-input" type="checkbox" name="food_ids[]" value="' . $foodId . '">' . $foodName;
                echo '</label><br>';
            }
            ?>
