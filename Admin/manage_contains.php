<html>
    ...
    <body>
    <?php
// Check if the form is submitted and the food IDs are present
if (isset($_POST['food_ids']) && isset($_POST['menu_id'])) {
    // Retrieve the selected food IDs and the menu ID
    ...
    // Prepare the query to check existing records in the 'contain' table
    $existingQuery = "SELECT * FROM contain WHERE menu_id = :menuId";

    // Prepare the query to insert new records into the 'contain' table
    $insertQuery = "INSERT INTO contain (menu_id, food_id) VALUES (:menuId, :foodId)";

    // Prepare the query to delete existing records from the 'contain' table
...
<!-- Start page content -->
    ...
    foreach ($existingResult as $existingRecord) {
        ...
        // Check if the existing food ID is in the selected food IDs array
        if (in_array($existingfoodId, $foodIds)) {
            // food ID exists in the selected food IDs array, do nothing
            continue;
        } else {
            // food ID does not exist in the selected food IDs array, delete the record
    ...
    // Loop through the selected food IDs
    foreach ($foodIds as $foodId) {
        // Check if the record exists in the 'contain' table
        ...
        if (count($existingRecord) > 0) {
            ...
            // Record doesn't exist, insert it into the 'contain' table
