<html>
    ...
    $query_upcoming = "SELECT * FROM hall natural join books natural join customer WHERE (customer_id=:customer_id and booking_date > SYSDATE and lower(status)='booked')";

    $statement_upcoming = oci_parse($connection, $query_upcoming);
    oci_bind_by_name($statement_upcoming, ':customer_id', $customer_id);
    oci_execute($statement_upcoming);

    // Generate HTML table with hall information
    $tableHtmlUpcoming = '<h3> Upcoming Bookings <h3><br>';
    if (($row_count = oci_fetch_all($statement_upcoming, $rows_upcoming)) > 0) {
        ...
        $tableHtmlUpcoming .= '<table>
            ...
            while ($row = oci_fetch_assoc($statement_upcoming)) {
                $tableHtmlUpcoming .= '<tr>';
                $tableHtmlUpcoming .= '<td style="text-align: left;">' . $row['HALL_NAME'] . '</td>';
                $tableHtmlUpcoming .= '<td style="text-align: left;">' . $row['BOOKING_DATE'] . '</td>';
                $tableHtmlUpcoming .= '<td style="text-align: left;">' . $row['BOOKED_SLOT'] . '</td>';
                $tableHtmlUpcoming .= '<td>
                    <form action="show_booking_details.php" method="post">
                        <input type="hidden" name="hall_id" value="' . $row['HALL_ID'] . '">
                        <input type="hidden" name="booking_date" value="' . $row['BOOKING_DATE'] . '">
                        <input type="hidden" name="booked_slot" value="' . $row['BOOKED_SLOT'] . '">
        ...
        $tableHtmlUpcoming .= '<p>No upcoming bookings.</p>';
    }

    oci_free_statement($statement_upcoming);

    $query_past = "SELECT * FROM hall natural join books natural join customer WHERE (customer_id=:customer_id and booking_date < SYSDATE and lower(status)='booked')";

    $statement_past = oci_parse($connection, $query_past);
    oci_bind_by_name($statement_past, ':customer_id', $customer_id);
    oci_execute($statement_past);

    // Generate HTML table with hall information
    $tableHtmlPast = '<h3> Past Bookings <h3><br>';
    if (($row_count = oci_fetch_all($statement_past, $rows_past)) > 0) {
        ...
        $tableHtmlPast .= '<table>
            ...
            while ($row = oci_fetch_assoc($statement_past)) {
                $tableHtmlPast .= '<tr>';
                $tableHtmlPast .= '<td style="text-align: left;">' . $row['HALL_NAME'] . '</td>';
                $tableHtmlPast .= '<td style="text-align: left;">' . $row['BOOKING_DATE'] . '</td>';
                $tableHtmlPast .= '<td style="text-align: left;">' . $row['BOOKED_SLOT'] . '</td>';
                $tableHtmlPast .= '<td>
                            <div class="button-container">
                                <form action="show_past_booking_details.php" method="post">
                                    <input type="hidden" name="hall_id" value="' . $row['HALL_ID'] . '">
                                    <input type="hidden" name="booking_date" value="' . $row['BOOKING_DATE'] . '">
                                    <input type="hidden" name="booked_slot" value="' . $row['BOOKED_SLOT'] . '">
                                    <input type="submit" value="View Details">
            ...
            $tableHtmlPast .= '<p>No past bookings.</p>';
        }

    oci_free_statement($statement_past);

    $query_cancelled = "SELECT * FROM hall natural join books natural join customer WHERE (customer_id=:customer_id and lower(status)='cancelled')";

    $statement_cancelled = oci_parse($connection, $query_cancelled);
    oci_bind_by_name($statement_cancelled, ':customer_id', $customer_id);
    oci_execute($statement_cancelled);

    $tableHtmlCancelled = '<h3> Cancelled Bookings <h3><br>';
    if (($row_count = oci_fetch_all($statement_cancelled, $rows_cancelled)) > 0) {
        ...
        $tableHtmlCancelled .= '<table>
            ...
            while ($row = oci_fetch_assoc($statement_cancelled)) {
                $tableHtmlCancelled .= '<tr>';
                $tableHtmlCancelled .= '<td style="text-align: left;">' . $row['HALL_NAME'] . '</td>';
                $tableHtmlCancelled .= '<td style="text-align: left;">' . $row['BOOKING_DATE'] . '</td>';
                $tableHtmlCancelled .= '<td style="text-align: left;">' . $row['BOOKED_SLOT'] . '</td>';
                $tableHtmlCancelled .= '<td>
                                <form action="show_cancelled_booking_details.php" method="post">
                                    <input type="hidden" name="hall_id" value="' . $row['HALL_ID'] . '">
                                    <input type="hidden" name="booking_date" value="' . $row['BOOKING_DATE'] . '">
                                    <input type="hidden" name="booked_slot" value="' . $row['BOOKED_SLOT'] . '">
                                    <input type="submit" value="View Details">
            ...
            $tableHtmlCancelled .= '<p>No cancelled bookings.</p>';
        }

    oci_free_statement($statement_cancelled);
                    ...
                    <div class="collapse navbar-collapse" id="fablesNavDropdown">
                        <ul class="navbar-nav mx-auto fables-nav">
                            <li class="nav-item dropdown">
                                <a class="nav-link" href="home4.html" id="sub-nav1" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                    Home
                                </a>
                            </li>
...
<!-- Start Header -->
<div class="fables-header fables-after-overlay" style="background-image: url(assets/custom/images/index1bg.jpg);">
    <div class="container"> 
         <h2 class="fables-page-title fables-second-border-color">Book History</h2>
    </div>
</div>  
<!-- /End Header -->
    ...
    <div class="container"> 
        <nav aria-label="breadcrumb">
          <ol class="fables-breadcrumb breadcrumb px-0 py-3">
            <li class="breadcrumb-item"><a href="Customer.html" class="fables-second-text-color">Home</a></li>
            <li class="breadcrumb-item active" aria-current="page">Book History</li>
          </ol>
        </nav> 
    </div>
</div>
...
<!-- Start page content -->
        ...
        <?php echo $tableHtmlUpcoming; ?><br><br>
        <?php echo $tableHtmlPast; ?><br><br>
        <?php echo $tableHtmlCancelled; ?><br><br>
