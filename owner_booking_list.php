<html>
    ...
    $query_upcoming = "SELECT * FROM hall natural join books natural join owns natural join owner WHERE (owner_id=:owner_id and booking_date > SYSDATE and lower(status)='booked')";

    $statement_upcoming = oci_parse($connection, $query_upcoming);
    oci_bind_by_name($statement_upcoming, ':owner_id', $owner_id);
    ...
    $query_past = "SELECT * FROM hall natural join books natural join owns natural join owner WHERE (owner_id=:owner_id and booking_date < SYSDATE and lower(status)='booked')";

    $statement_past = oci_parse($connection, $query_past);
    oci_bind_by_name($statement_past, ':owner_id', $owner_id);
    ...
    if (($row_count = oci_fetch_all($statement_past, $rows_past)) > 0) {
        ...
        $tableHtmlPast .= '<table>
            ...
            while ($row = oci_fetch_assoc($statement_past)) {
                ...
                $tableHtmlPast .= '<td>
                                <form action="show_cancelled_booking_details_owner.php" method="post">
                                    <input type="hidden" name="hall_id" value="' . $row['HALL_ID'] . '">
                                    <input type="hidden" name="booking_date" value="' . $row['BOOKING_DATE'] . '">
                                    <input type="hidden" name="booked_slot" value="' . $row['BOOKED_SLOT'] . '">
                                    <input type="submit" value="View Details">
    ...
    $query_cancelled = "SELECT * FROM hall natural join books natural join owns natural join owner WHERE (owner_id=:owner_id and lower(status)='cancelled')";

    $statement_cancelled = oci_parse($connection, $query_cancelled);
    oci_bind_by_name($statement_cancelled, ':owner_id', $owner_id);
        ...
        <div class="row">
            <div class="col-12 col-md-10 col-lg-9 pr-md-0">
                <nav class="navbar navbar-expand-md btco-hover-menu py-lg-2">
                    <a class="navbar-brand" href="owner.html">
    ...
    <div class="container"> 
        <nav aria-label="breadcrumb">
          <ol class="fables-breadcrumb breadcrumb px-0 py-3">
            <li class="breadcrumb-item"><a href="owner.html" class="fables-second-text-color">Home</a></li>
